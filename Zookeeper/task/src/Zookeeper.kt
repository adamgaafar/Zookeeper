    ...
    while(!end){
        ...
        when(input){
            "exit" -> {
                ...
                if (!isInRange(input.toInt(),0..animals.size)) {
                    println("\"Please enter a number within [0 .. ${animals.size - 1}]: \"")
                }else {
                    print(animals[input.toInt()])
                }
            }
        }
    }
...
fun isInRange(number:Int, animalRange: IntRange): Boolean{
    return animalRange.contains(number)
}